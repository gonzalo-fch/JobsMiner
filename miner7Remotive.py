...
def limpiar_datos(data):
    ...
    # 👇 AQUÍ cambia todo respecto a RemoteOK
    jobs = data.get("jobs", [])
    ...
    for item in jobs:
        job = {
            ...
            "ubicacion": item.get("candidate_required_location"),
            "categoria": item.get("category"),
            "tipo": item.get("job_type"),
            "tags": item.get("tags"),
...
def guardar_json(data, archivo="remotive_jobs.json"):
    # Compatible Windows (tu caso)
